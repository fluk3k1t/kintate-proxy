use crate::policy::{Policy, RuleData, Action};
...
impl LimitManager {
    pub fn new(policy: Policy) -> SqliteResult<Self> {
        let conn = policy.get_connection();
    ...
    pub fn enforce(&self) -> SqliteResult<()> {
        ...
        let today_start_str = DateTime::<Local>::from_naive_utc_and_offset(today_start, *now.offset()).to_rfc3339();
        ...
        for limit in limits {
            ...
            if usage >= limit.max_duration_secs {
                info!("Limit exceeded for tag '{}': current={}s max={}s. Blocking...", 
                    limit.tag, usage, limit.max_duration_secs);
                
                // Add block rule if not already present
                let rules = self.policy.get_all_rules();
                let already_blocked = rules.iter().any(|r| r.data.name.as_deref() == Some(&rule_name));
