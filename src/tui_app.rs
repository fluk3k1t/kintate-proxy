use crate::policy::{Policy, Rule, AccessSession, DomainTag, Action, RuleData};
...
impl App {
        ...
        loop {
            ...
            if event::poll(timeout)? {
                if let Event::Key(key) = event::read()? {
                    // Only handle press events to avoid double input on some terminals
        ...
        match self.active_tab {
            Tab::Rules => {
                ...
                self.form_fields = vec![
                    ("Priority (e.g. 10)".to_string(), "100".to_string()),
                    ("Action (Allow/Block)".to_string(), "Block".to_string()),
                    ("Domain (optional)".to_string(), "".to_string()),
                    ("Tag (optional)".to_string(), "".to_string()),
                    ("Name (optional)".to_string(), "".to_string()),
                    ("[Submit]".to_string(), "Press Enter".to_string()),
                ];
            }
            Tab::Tags => {
                self.input_mode = InputMode::AddingTag;
                self.form_fields = vec![
                    ("SLD (e.g. youtube)".to_string(), "".to_string()),
    ...
    fn refresh_data(&mut self) {
        ...
        self.logs = self.policy.get_access_logs(50, None, None).unwrap_or_default();
    ...
    fn ui(&self, f: &mut ratatui::Frame) {
        ...
        let tabs = Tabs::new(titles)
            .block(Block::default().borders(Borders::ALL).title(" Kintate Proxy Manager "))
            .select(Tab::all().iter().position(|&t| t == self.active_tab).unwrap());
        ...
        let help_text = if self.input_mode == InputMode::Viewing {
            " [Q] Quit  [Tab] Next Tab  [R] Refresh  [A] Add ".to_string()
        } else {
            " [Esc] Cancel  [Tab] Next Field  [Enter] Next/Submit ".to_string()
        };
    ...
    fn render_popup(&self, f: &mut ratatui::Frame, area: Rect) {
        ...
        let inner_chunks = Layout::default()
            ...
            .margin(2)
            .constraints(
                self.form_fields.iter().map(|_| Constraint::Length(3)).collect::<Vec<_>>()
            )
            .split(popup_area);
    ...
    fn render_rules(&self, f: &mut ratatui::Frame, area: Rect) {
    ...
    fn render_logs(&self, f: &mut ratatui::Frame, area: Rect) {
        ...
        .header(header)
        .block(Block::default().borders(Borders::ALL).title(" Session Activity (Flushed) "));
        f.render_widget(t, area);
    }

    fn render_tags(&self, f: &mut ratatui::Frame, area: Rect) {
        ...
        .block(Block::default().borders(Borders::ALL).title(" Domain -> Tag Mappings "));
        f.render_widget(t, area);
    }

    fn render_limits(&self, f: &mut ratatui::Frame, area: Rect) {
